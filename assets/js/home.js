      ...
      if (latestPosts.length) {
        postsGrid.innerHTML = latestPosts.map((post, i) => {
          ...
          return `<a class="dispatch-card" href="profile.html?photographer=${encodeURIComponent(post.photographer)}" data-reveal style="--delay:${i * 60}ms">
            ...
            <div class="dispatch-card__copy"><div class="dispatch-card__author"><div class="creator-avatar creator-avatar--xs">${avatar(person)}</div><span>${esc(post.photographerName)}<small>${esc(post.date)}</small></span></div><h3>${esc(post.title)}</h3><p>${esc(post.body)}</p><b>Read on profile ↗</b></div>
          </a>`;
        }).join('');

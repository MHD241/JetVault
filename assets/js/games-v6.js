(() => {
  ...
  const achievements = [
    ...
    { id: 'allrounder', name: 'Type Rated', text: 'Play all eight Games Lab challenges.', check: () => ['logos','models','codes','makers','regs','wingspan','firstflight','northbound'].every(id => (state.bests[id] || {}).played) },
  ...
  function openGame(id) {
    ...
    const runners = { logos: runLogoQuiz, models: runModelQuiz, codes: runCodeSprint, makers: runMakerRush, regs: runRegistrationQuiz, wingspan: runWingspanBattle, firstflight: runFirstFlightDuel, northbound: runNorthbound };
    runners[id]?.();
  }
  function closeGame() {
    activeCleanup?.(); activeCleanup = null;

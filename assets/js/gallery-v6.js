(() => {
  ...
  function card(photo) {
    return `<article class="photo-card" id="photo-${esc(photo.id)}">
      <button class="photo-card__open" type="button" data-photo-id="${esc(photo.id)}" aria-label="Open ${esc(photo.airline)} ${esc(photo.aircraft)}">
      ...
      <div class="photo-card__social" data-card-social="${esc(photo.id)}">
        <button class="photo-card__quick-like" type="button" data-quick-like="${esc(photo.id)}" aria-label="Like this photograph"><span>♡</span><b data-quick-like-count>0</b></button>
  ...
  function render() {
    ...
    const syncCard = (id, state) => {
      const node = grid.querySelector(`[data-card-social="${CSS.escape(String(id))}"]`);
      if (!node || !state) return;
      const like = node.querySelector('[data-quick-like]');
      const icon = like?.querySelector('span');
      const likes = node.querySelector('[data-quick-like-count]');
      const comments = node.querySelector('[data-quick-comment-count]');
      if (likes) likes.textContent = state.likes ?? 0;
      if (comments) comments.textContent = Array.isArray(state.comments) ? state.comments.length : (state.comments ?? 0);
      if (like) like.classList.toggle('is-active', Boolean(state.liked));
      if (icon) icon.textContent = state.liked ? '♥' : '♡';
    };
    if (social && backend.configured) {
      social.getCounts(list.map(p => p.id)).then(counts => {
        grid.querySelectorAll('[data-card-social]').forEach(node => {
          const c = counts[node.dataset.cardSocial] || { likes:0, comments:0, liked:false };
          const likes = node.querySelector('[data-quick-like-count]');
          const comments = node.querySelector('[data-quick-comment-count]');
          const like = node.querySelector('[data-quick-like]');
          const icon = like?.querySelector('span');
          if (likes) likes.textContent = c.likes;
          if (comments) comments.textContent = c.comments;
          if (like) like.classList.toggle('is-active', Boolean(c.liked));
          if (icon) icon.textContent = c.liked ? '♥' : '♡';
        });
      }).catch(() => {});
      grid.querySelectorAll('[data-quick-like]').forEach(btn => btn.addEventListener('click', async e => {
        e.preventDefault(); e.stopPropagation(); btn.disabled = true;
        try {
          const state = await social.toggleLike(btn.dataset.quickLike);
          if (state) { syncCard(btn.dataset.quickLike, state); btn.classList.remove('just-liked'); void btn.offsetWidth; btn.classList.add('just-liked'); }

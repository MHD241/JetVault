(() => {
  ...
  function render(data) {
    grid.innerHTML = data.photographers.map((person, i) => {
      ...
      return `<a class="creator-card" href="${profileHref(person)}" data-reveal data-tilt style="--delay:${i * 70}ms">
        ...
        <div class="creator-card__body">
          ...
          <p>${esc(person.bio)}</p>
          <div class="creator-card__stats"><span><b>${shots.length}</b> photos</span><span><b>${posts.length}</b> posts</span><span><b>${airports}</b> airports</span><span data-directory-followers="${esc(person.accountId || '')}"><b>0</b> followers</span></div>
    ...
    if (social && backend.configured) grid.querySelectorAll('[data-directory-followers]').forEach(async node => { if (!node.dataset.directoryFollowers) return; const state = await social.getFollowState(node.dataset.directoryFollowers).catch(() => null); if (state) node.innerHTML = `<b>${state.followers}</b> followers`; });
  }

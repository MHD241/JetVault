(() => {
  ...
  async function getCrewAccounts({ fresh = false } = {}) {
    if (!fresh && accountsPromise) return accountsPromise;
    const task = (async () => {
      const db = await ensureClient();
      if (!db) return [];
      const { data, error } = await db.from('profiles').select('id,display_name,is_manager,created_at').order('created_at', { ascending: true });
      if (error) throw error;
      return data || [];
    })().catch(() => []);
    if (!fresh) accountsPromise = task;
    return task;
  }

  function mergePhotographers(profileRows = [], accountRows = []) {
    ...
    const accountByName = new Map(accountRows.map(row => [row.display_name, row]));
    return (fallback.photographers || []).map(person => {
      const meta = latestByPerson.get(person.id);
      const account = accountByName.get(person.name);
      return {
        ...person,
        ...
        isManager: Boolean(account?.is_manager)
      };
    });
  }

  async function getData({ fresh = false } = {}) {
    const [rows, accounts] = await Promise.all([getRows({ fresh }), getCrewAccounts({ fresh })]);
    if (!rows) return {
    ...
    return {
      photographers: mergePhotographers(profileRows, accounts),
      airports: fallback.airports || [],
  ...
  function invalidateContent() { contentPromise = null; accountsPromise = null; }
